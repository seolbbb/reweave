...
|---|---|---|---|---|---|
| 2026-07-21 | Integrate completed work through `dev` into `main` | Maintenance / Other | In progress | Preserve the existing onboarding/archive-management commit and the completed Phase 0 memory-audit pilot as separate history, update the existing PR to `dev`, require merge commits, then merge the verified integration branch into `main`; no new product scope | Local pre-merge gates passed: Ruff; 110 Python tests; 19 frontend tests; TypeScript and production frontend build; fresh PyInstaller build created `dist/Reweave/Reweave.exe`; 8-second executable startup smoke passed. GitHub CI and branch integration remain pending |
